...
export function PortfolioList({ onEdit, onNew, onDelete: onDeleteProp }: PortfolioListProps) {
  ...
  const handleCopyLink = (slug: string) => {
    const url = `https://binaproject.id/portfolio/${slug}`;
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="p-4">
          ...
          <div className="mt-2 text-2xl font-semibold tracking-tight text-slate-900 font-mono tabular-nums">
            {loading ? <Skeleton className="h-7 w-14 my-0.5" /> : projects.length}
          </div>
          <p className="text-[11px] text-slate-500 mt-1 font-medium">Tersimpan di database</p>
        </Card>

        <Card className="p-4">
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Terpublikasi (Live)</p>
          ...
          <p className="text-[11px] text-slate-500 mt-1 font-medium">Aktif tampil di website publik</p>
        </Card>

        <Card className="p-4">
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Draft Tersimpan</p>
            <div className="p-1.5 rounded-md bg-amber-50 text-amber-600">
              <Clock className="w-4 h-4" />
            </div>
          </div>
          <div className="mt-2 text-2xl font-semibold tracking-tight text-slate-700 font-mono tabular-nums">
            {loading ? <Skeleton className="h-7 w-14 my-0.5" /> : draftCount}
          </div>
          <p className="text-[11px] text-slate-500 mt-1 font-medium">Belum ditayangkan ke publik</p>
        </Card>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50 text-slate-600 font-semibold border-b border-slate-100 text-xs uppercase tracking-wider">
                <tr>
                  <th className="py-3.5 px-4 font-semibold">Proyek</th>
                  <th className="py-3.5 px-4 font-semibold">Alamat Tautan (URL)</th>
                  <th className="py-3.5 px-4 font-semibold">Kategori</th>
                  <th className="py-3.5 px-4 font-semibold">Lokasi</th>
                  ...
                  <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        <a
                          href={`https://binaproject.id/portfolio/${item.slug}`}
                          target="_blank"
            ...
            <Card key={item.id} className="overflow-hidden flex flex-col justify-between group hover:border-slate-300 transition-colors">
              <div>
                ...
                <div className="p-4 space-y-1.5">
                  ...
                  <h3 className="font-semibold text-sm text-slate-900 line-clamp-1 group-hover:text-[#1B365D] transition-colors">
                    {item.title}
                  </h3>
