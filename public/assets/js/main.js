(function ($) {
  ...
  /*---------- 01. On Load Function ----------*/
    ...
    // Only initialize jQuery mobile menu if native custom-mobile-menu component is absent
    if (!$(".custom-mobile-menu").length) {
    ...
    /*---------- 05. Scroll To Top ----------*/
    // Handled natively by modern ScrollToTop.astro with Lenis smooth-scroll integration
    if (false && $('.scroll-top').length > 0) {
    ...
    $('.th-slider').each(function () {
        ...
        var options = {};
        try {
            var rawOptions = thSlider.attr('data-slider-options');
            if (rawOptions) {
                options = JSON.parse(rawOptions);
            }
        } catch (e) {
            console.warn('Invalid slider options JSON on', thSlider.get(0), e);
        }
        options = $.extend({}, sliderDefault, options);
        if (!thSlider.get(0).swiper) {
    ...
    // Only initialize jQuery sidemenu if modern native SideMenu.astro is absent
    if (!$('.sidemenu-wrapper .modern-sidemenu').length) {
