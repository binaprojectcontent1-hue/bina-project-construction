/**
 * Build-time HTML Sanitizer & Intelligent Editorial Formatter
 * Handles both rich HTML from CMS/WYSIWYG and raw plain-text/markdown drafts,
 * ensuring articles always render with proper paragraphs, headings, lists, and hierarchy.
 */
...
/**
 * Formats inline markdown and typographic emphasis (bold, italic, trik/tips highlight).
 */
...
/**
 * Automatically converts raw plain-text / unformatted drafts into semantic HTML.
 * Handles headings (e.g. "1. Sesuaikan..."), lists, callouts, and paragraphs.
 */
export function formatPlainTextToHtml(raw: string): string {
  ...
  for (let block of rawBlocks) {
    ...
    if (mdHeading) {
      const level = Math.min(mdHeading[1].length + 1, 4); // # -> h2, ## -> h3, ### -> h4
...
/**
 * Main entry point:
 * - If content has HTML block tags (<p>, <div>, <h1>-<h6>, etc.), sanitizes and returns.
 * - If content is plain text / markdown, formats into clean semantic HTML and sanitizes.
 */
export function formatAndSanitizeArticleContent(raw: string): string {
  ...
  const formattedHtml = formatPlainTextToHtml(raw);
